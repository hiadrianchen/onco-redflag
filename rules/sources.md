...
| `acs_when_to_call` | ACS《Chemotherapy Side Effects》"何时联系治疗团队 / 急救"（**通用框架**，EMG-001 意识改变等通用急症） | American Cancer Society. https://www.cancer.org/cancer/managing-cancer/treatment-types/chemotherapy/chemotherapy-side-effects.html |
| `acs_resp_emergency` | ACS 同上 — **呼吸/循环急症簇**（EMG-002 气促/胸痛/发绀） | 同 ACS 链接，§2 EMG-002 摘录"trouble breathing → call 911 first" |
| `acs_bleeding` | ACS 同上 + NCI — **出血急症簇**（EMG-003 出血/呕血/黑便/血尿） | ACS"Bleeding…/Bloody stool or blood in urine" + NCI"urine that is bloody"，§2 EMG-003 |
| `nccn_gi_management` | **消化道脱水簇**（GI-001 呕吐/腹泻）：定性 ACS/NCI；中文专属共识见 KnowS 缓存（恶心呕吐/腹泻全程管理专家共识）；数值参 CTCAE | ACS/NCI 链接同上；中文共识经 prefetch vendored 进 knows_cache.yaml |

> 注 1：原 `acs_when_to_call` 一源覆盖「意识/呼吸/出血/消化道」四个症状簇，**已按症状簇拆分**为
> `acs_when_to_call`（通用/意识）、`acs_resp_emergency`（呼吸）、`acs_bleeding`（出血）、`nccn_gi_management`（消化道），
> 各挂本簇已核证的 ACS/NCI 原文（非新增编造；snippet 即 §2 逐规则已实查的原文要点），中文专属共识由 KnowS prefetch 补充。
> 注 2：`redflags.yaml` 历史上的 `source: nccn_thrombocytopenia / idsa_clabsi` 已并入上述实查来源
...
| **EMG-002** 气促/胸痛/发绀→RED | ACS：**"Shortness of breath or trouble breathing (If you're having trouble breathing, call 911 first.)"**。胸痛/发绀按通用急救常识纳入（≥气促同级处理）。 | **acs_resp_emergency**（ACS） | **validated** |
| **EMG-003** 出血/呕血黑便→RED | ACS：**"Bleeding or unexplained bruising"、"Bloody stool or blood in your urine"**；NCI：**"urine that is bloody…"**。呕血/黑便按急症纳入。 | **acs_bleeding**（ACS / NCI） | **validated** |
| **EMG-001** 意识改变→RED | 神志改变/抽搐属任何情况下的急救范畴（拨 120），通用急救常识；癌症患教未单列但不降级。 | acs_when_to_call（通用框架/急症标准） | **validated-operational** |
| **GI-001** 呕吐/腹泻致脱水→RED | 定性依据：ACS **"Long-lasting diarrhea or vomiting"、"Bloody stool"**；NCI 列 "diarrhea"。**数值为操作化**：腹泻 ≥6 次/日（参 CTCAE 腹泻分级，3 级≈较基线 ≥7 次/日，本工具取更保守的 ≥6）、呕吐无法进食/饮水 ≥24h（脱水风险）、血便。 | **nccn_gi_management**（ACS / NCI + CTCAE 参考；中文共识见 KnowS 缓存） | **validated-operational** |
